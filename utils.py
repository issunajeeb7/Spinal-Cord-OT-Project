...
class IoULoss(nn.Module):
    ...
    def _iou_loss(self, score, target):
        ...
        union = torch.sum(score) + torch.sum(target) - intersection
    ...
    def forward(self, inputs, target):
        ...
        for i in range(self.n_classes):
            ...
            class_wise_iou.append(iou.item())
        mean_iou = sum(class_wise_iou) / self.n_classes
        return mean_iou
...
def resize_pos_embed(posemb, grid_size_h, grid_size_w):
    # Rescale the grid of position embeddings when loading from a checkpoint.
    posemb_grid = posemb.reshape(1, grid_size_h, grid_size_w, -1).permute(0, 3, 1, 2)
    posemb_grid = F.interpolate(posemb_grid, size=(grid_size_h, grid_size_w), mode='bilinear', align_corners=False)
    posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(1, grid_size_h * grid_size_w, -1)
    return posemb_grid